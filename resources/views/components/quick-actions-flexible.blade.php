...
@if($compact)
    ...
    <div class="space-y-2">
        @foreach($actions as $action)
            <a href="{{ $action['url'] }}" 
               class="flex items-center p-3 rounded-lg text-gray-700 hover:text-gray-900 hover:bg-orange-50 border border-gray-200 hover:border-orange-300 transition-all duration-200 group">
                <i class="{{ $action['icon'] }} text-gray-500 group-hover:text-orange-600 mr-3 w-5 text-center"></i>
        ...
        <form method="POST" action="{{ route('logout') }}" class="w-full">
            @csrf
            <button type="submit" 
                    class="w-full flex items-center p-3 rounded-lg text-red-700 hover:text-red-900 hover:bg-red-50 border border-red-300 hover:border-red-500 transition-all duration-200 group">
                <i class="fas fa-sign-out-alt text-red-600 group-hover:text-red-700 mr-3 w-5 text-center"></i>
