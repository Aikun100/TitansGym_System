    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="glass-card rounded-xl p-6 mb-6">
            <div class="flex justify-between items-center mb-6">
                ...
                @if($memberReview)
                    <a href="{{ route('member.reviews.edit', $memberReview) }}" 
                       class="px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition text-sm">
                        <i class="fas fa-edit mr-1"></i>
                        Edit Your Review
                    </a>
                @else
                    <a href="{{ route('member.trainers.review.create', $trainer) }}" 
                       class="px-4 py-2 bg-gradient-to-r from-orange-600 to-red-600 text-white rounded-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200 text-sm">
            ...
            @if($totalReviews > 0)
                ...
                <div id="reviews-container" class="hidden space-y-4">
                    @foreach($reviews as $review)
                        <div class="bg-white bg-opacity-50 rounded-lg p-4 {{ $review->member_id === auth()->id() ? 'border-2 border-orange-300' : '' }}">
                            <div class="flex items-start justify-between mb-2">
                                ...
                                @if($review->member_id === auth()->id())
                                    <a href="{{ route('member.reviews.edit', $review) }}" 
                            ...
                            @if($review->review)
                                <p class="text-gray-700 text-sm mt-2 pl-13">{{ $review->review }}</p>
                            @endif
                        </div>
                    @endforeach
                </div>
            @else
                ...
                <div class="text-center py-8">
                    ...
                    <p class="text-gray-600 mb-4">No reviews yet. Be the first to review this trainer!</p>
                    <a href="{{ route('member.trainers.review.create', $trainer) }}" 
                       class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-orange-600 to-red-600 text-white rounded-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200">
